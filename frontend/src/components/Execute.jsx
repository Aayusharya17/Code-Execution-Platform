...
export default function Execute() {
  ...
  // files stored per-project: { [projectId]: [...files] }
  ...
  // inline input state: { projectId, type: 'file'|'folder' }
  ...
  const fetchFiles = async (projectId) => {
    try {
      ...
      // store files keyed by projectId so each project has its own list
  ...
  // Show inline input box for new file or folder
  ...
  const commitInlineCreate = async () => {
    ...
    try {
      if (type === "file") {
        ...
        // create folder — adjust endpoint to match your API
    ...
    <div style={{ display: "flex", height: "100vh", background: "#1e1e1e", color: "#cccccc", fontFamily: "'Segoe UI', system-ui, sans-serif", fontSize: "13px", overflow: "hidden", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: "48px", background: "#333333", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: "8px", gap: "4px", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }} />
          <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: "#007acc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 600, marginBottom: "8px", cursor: "pointer" }}>U</div>
        </div>
...
{/* Inline new-project input — appears just below the Explorer header */}
{newProjectInput && (
  ...
  if (e.key === "Enter") {
    e.preventDefault();
    setNewProjectInput(false); // close input immediately so onBlur is a no-op
  ...
  onBlur={() => { setNewProjectInput(false); setNewProjectName(""); }}
    ...
    />
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {/* Breadcrumbs */}
          {selectedFileName && (
            <div style={{ height: "22px", background: "#1e1e1e", display: "flex", alignItems: "center", padding: "0 12px", gap: "6px", fontSize: "12px", color: "#858585", flexShrink: 0, borderBottom: "1px solid #2d2d2d" }}>
              <span style={{ cursor: "pointer" }}>project</span>
              <span>›</span>
              <span style={{ color: "#cccccc" }}>{selectedFileName}</span>
            </div>
          )}
